...
echo 在第二步中粘贴到脚本中
echo.
echo tips:点击鼠标右键可以直接粘贴(无需Ctrl+V)"( ´ ▽ ` )ﾉ"
echo.
echo 请选择操作"~(=^‥^)ノ":
echo.
...
:setShutdown
echo.
echo 打开浏览器访问校园网登陆网址(可自行修改网址)?您是否要继续此操作"“ψ(｀∇´)ψ"?
echo 默认值为Y (Y/N?)
...
start "" %url%
if %errorlevel% neq 0 (
    echo 打开浏览器失败，请检查你的系统设置。"(+_+)"
    echo.
    goto :menu
)

echo 浏览器已成功打开，正在访问校园网登陆网址..."ヽ(´・｀)ﾉ"
goto :menu
...
echo 生成开机自动登陆脚本？
echo.
echo 注意:再次生成会覆盖旧的文件！！！！您是否要继续此操作"(*´ω｀)o"?
echo 默认值为Y (Y/N?)
echo ====================================================
...
set /p url=
if "%url%"=="" (
    echo 你没有输入url,请重新输入"(×_×#"
    goto :createScript
)


echo 是否需要添加 pause？ 添加会在登陆成功后保留结果窗口(一般由于调试)"(○’ω’○)"
echo 默认值为N (Y/N)
set "addPause="
set /p addPause=
...
echo echo 正在登录校园网..."（ゝ。∂）" >> %scriptPath%
echo echo. >> %scriptPath%
...
echo     echo 登录成功"（*＾ω＾）人（＾ω＾*）" >> %scriptPath%
echo     echo. >> %scriptPath%
...
echo 正在将文件复制到系统自启动目录"ヾ (o ° ω ° O ) ノ゙"
copy "%source%" "%destination%"
REM 检查文件是否成功复制
if exist "%destination%" (
    echo 文件成功复制到目标路径"ヽ( °◇°)ノ"
) else (
...
echo.
set "choice="
set /p choice=是否删除原文件?默认值为Y (输入Y/N？) "(ノ^∇^)"
if "%choice%"=="" set "choice=Y"
echo.
